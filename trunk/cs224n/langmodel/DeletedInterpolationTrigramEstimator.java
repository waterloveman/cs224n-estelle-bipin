...
public class DeletedInterpolationTrigramEstimator implements NgramProbabilityEstimator	{
	
	private NgramCounter ngc;
	private NgramProbabilityEstimator baseEstimator;
	
	public DeletedInterpolationTrigramEstimator(NgramCounter n, NgramProbabilityEstimator baseEst)	{
		ngc = n;
	...
	public double getNgramJointProbability(List<String> ngram){
		...
		if (ngram.size() == 1){
			return getNgramJointProbability(ngram);
		}
		if (ngram.size() == 2){
			return getNgramJointProbability(ngram);
		}
	...
	public double getNgramConditionalProbability(List<String> ngram)	{
		...
		if (ngram.size() == 1){
			...
			ArrayList<String> n_minus_2_gram = new ArrayList(n_minus_1_gram);
			n_minus_2_gram.remove(0);
			...
			return 	lambda1 * baseEstimator.getNgramConditionalProbability(ngram) +
					...
					lambda3 * baseEstimator.getNgramConditionalProbability(n_minus_2_gram);
		}
	}
